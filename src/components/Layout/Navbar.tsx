...
export default function Navbar({ showGetInTouch = true, navbar = "secondary" }: NavbarProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setScrollState(prev => ({
        isSticky: currentScrollY < prev.lastScrollY && currentScrollY > 50,
    ...
    <AnimatePresence>
      {isMenuOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[99999] overflow-y-auto h-full bg-white"
  ...
  return (
    <div>
      <header 
        className={`max-w-[3000px] pb-3 2xl:max-w-full 2xl:mx-40 xl:pt-[98px] xl:px-0 xl:mx-auto lg:pt-[62px] lg:px-[48px] lg2:px-[78px] sm:pt-[34px] sm:px-[26px] pt-[34px] px-[26px] transition duration-1000 ${
          scrollState.isSticky 
      ...
      >
        <nav className="flex flex-col items-center lg:flex-row w-full">
          <div className="w-full lg:w-1/2 flex items-center justify-start">
            <Link href="/">
              <Image
                src={scrollState.isSticky ? logo : (isNavbarPrimary ? logoWhite : logo)}
                alt="Logo"
            ...
            <div className="flex items-center cursor-pointer ml-auto lg:hidden" onClick={() => setIsMenuOpen(!isMenuOpen)}>
              {scrollState.isSticky || isNavbarPrimary ? <MenuIconWhite /> : <MenuIcon />}
            </div>
          </div>
                ...
                <div key={href} className="relative group">
                  <div onMouseEnter={() => hasDropdown && setActiveDropdown(href)} onMouseLeave={() => setActiveDropdown(null)}>
                    ...
                    {hasDropdown && activeDropdown === href && (
                      <div className="group">
                        <div className="absolute left-0 w-40 mt-1 origin-top-left bg-transparent bg-opacity-80 backdrop-blur-3xl rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition duration-300 z-50">
                          {dropdownItems?.map((item) => (
                ...
                <Link href="/general-enquire">
                  <Button
                    className={`w-full pt-[2px] text-base sm:text-lg md:text-xl lg2:text-2xl px-4 lg2:px-7 xl:px-10 lg:text-[20px] xl:text-[26px] 2xl:text-4xl ${
                      scrollState.isSticky ? "text-white" : isNavbarPrimary ? "bg-white" : ""
                    }`}
                    defaultTextColor={scrollState.isSticky ? "text-white" : buttonColor}
                  >
                    Get in Touch
                  </Button>
                </Link>
