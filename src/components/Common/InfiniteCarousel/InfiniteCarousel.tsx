        ...
        <div key={"dummy-content"} data-lenis-prevent>
          ...
          <div className="p-4 md:p-10">
            <Typography variant="h1" className="text-customBrown">
             ...
             <Typography variant="h1" className="text-customBrown mt-2">
              {project?.bottomTitle2?.split("").map((char, index) => (
                <span key={index} className={`${/\d/.test(char) ? "font-CandideCondensedMedium !text-2xl" : "!text-2xl font-freightNeoMedium"}`}>
                  {char}
                </span>
              ))}
            </Typography>
