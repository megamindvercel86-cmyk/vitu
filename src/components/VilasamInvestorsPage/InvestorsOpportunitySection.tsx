      ...
      <div className="mx-auto max-w-7xl xl:max-w-[90vw]">
        <div className="grid pb-5 md:pb-0 grid-cols-1 gap-6 md:grid-cols-[1fr_1.4fr] md:items-center md:gap-10 lg:gap-16">
          ...
          <div className="order-1 flex flex-col space-y-4 md:order-1 md:space-y-7">
            <h2 className="text-balance font-ttCommons leading-tight font-semibold text-[28px] text-[#2A2A2A] sm:text-[32px] md:text-4xl lg:text-5xl">
              {heading}
            </h2>
            <p className="max-w-sm pb-8 font-ttCommons fontdemi text-[15px] leading-relaxed text-[#999999] md:text-lg lg:max-w-xl">{description}</p>
            <Link
              href={ctaHref}
              className="hidden h-12 w-fit items-center justify-center rounded-md bg-[#064747] px-8 font-ttCommons text-sm font-semibold text-white transition hover:bg-[#084943] md:inline-flex md:h-12 md:px-10 md:text-base"
            >
              {ctaLabel}
            </Link>
          </div>
          ...
          <div className="order-3 mt-4 flex justify-center md:hidden">
            <Link
              href={ctaHref}
              className="inline-flex py-2 md:h-12 w-full items-center justify-center rounded-[4px] bg-[#064747] px-8 font-ttCommons text-[15px] font-bold tracking-wide text-white transition hover:bg-[#084943]"
            >
              {ctaLabel}
            </Link>
          </div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 md:mt-12 md:grid-cols-2 lg:mt-16 xl:gap-44 lg:grid-cols-4 lg:gap-6">
          {cards.map((card) => (
            <div
              ...
              className="flex flex-col justify-center rounded-[10px] bg-[#F9F9F9] px-4 py-6 text-center md:min-h-[120px] md:px-5"
            >
              <p className="md:whitespace-pre-line font-ttCommons text-lg font-bold leading-snug text-[#2A2A2A] md:text-xl lg:text-2xl">
                {card.title}
              </p>
