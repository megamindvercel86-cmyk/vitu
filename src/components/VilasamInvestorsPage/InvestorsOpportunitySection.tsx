...
export default function InvestorsOpportunitySection({
  ...
  return (
    <section className="bg-white px-6 pb-16 pt-7 md:px-12 md:py-20 lg:px-20 lg:py-24">
      <div className="mx-auto max-w-7xl xl:max-w-[90vw]">
        <div className="grid pb-5 md:pb-0 grid-cols-1 gap-6 md:grid-cols-[1fr_1.4fr] md:items-center md:gap-10 lg:gap-16">
          ...
          <div className="order-1 flex flex-col space-y-4 md:order-1 md:space-y-7">
            <h2 className="text-balance font-ttCommons leading-tight font-semibold hidden text-[28px] text-[#2A2A2A] sm:text-[32px] md:text-4xl lg:text-5xl md:block">
              {heading}
            </h2>
          ...
          <div className="order-3 mt-4 flex justify-center md:hidden">
            <Link
              href={ctaHref}
              className="inline-flex py-2 md:h-12 w-full items-center justify-center rounded-[4px] bg-[#064747] px-8 font-ttCommons text-[16px] font-bold tracking-wide text-white transition hover:bg-[#084943]"
            >
              {ctaLabel}
            </Link>
          </div>
