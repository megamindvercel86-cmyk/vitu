...
import VideoPlayer, { VideoPlayerRef } from "@/components/Common/VideoPlayer/page";
import NewVideoPlayer from "@/components/Common/NewVideoPlayer/NewVideoPlayer";
...
export default function VilasamCarousel(): React.ReactElement {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ...
      { threshold: 0 } // Pause as soon as it's fully out of view (or starting to leave if threshold 0)
    );
  ...
  return (
    <section ref={sectionRef} className="bg-gradient-to-b">
      ...
      <div className="rounded-md flex-col antialiased items-center justify-center relative overflow-hidden">
        <div className="flex flex-col md:gap-12 gap-4  text-center">
          <p className=" text-[#0C3E49CC] mx-auto font-ttCommons md:text-xl  lg2:text-2xl lg:text-xl text-base px-4">
          ...
          <h2 className="text-2xl md:text-3xl lg:text-5xl lg2:text-6xl  md:max-w-5xl md:mx-3 lg:mx-auto font-medium font-theSeasons text-[#0C3E49]">
      ...
      {/* <Link href="/general-enquire">
        <div className="flex justify-center mt-16 md-2">
          <button aria-label="Contact Us for a Visit" className="px-8 py-3 border-[#0C3E49] border-[2px] text-[#0C3E49] rounded-full lg2:text-2xl font-theSeasons transition-colors">
